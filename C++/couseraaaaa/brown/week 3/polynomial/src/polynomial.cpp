...
public:
 ...
 // int Degree() const {
  ...
  class helper{
  public:
 	  explicit helper(size_t d, Polynomial& pol):degree(d),p(pol){}

 	 // template <typename T>
 	    T& operator = (const T& value){
 		  if(degree > static_cast<size_t>(p.Degree())){
 			  p.coeffs_.resize(degree+1);
 		  }
 		  p.coeffs_[degree]=value;
 		  p.Shrink();
 		  return degree<static_cast<size_t>(p.Degree())?p.coeffs_[degree]:zero;
 	    ...
 	    operator T() const {
 	    	...
 	    	return p.coeffs_[degree];
 	    	/*if(degree>static_cast<size_t>(p.Degree())){
 	    	 	    		return 0;
 	    	 	    	}
 	    	 	return std::as_const(p)[degree];*/
 	    }
  ...
  helper operator [](size_t degree)  {
    return helper(degree,*this);
}
