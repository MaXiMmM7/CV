...
public:
  ...
  WriteAccess operator[](const K& key){
	auto& need=book[hasher(key)%book.size()];
	return {lock_guard<mutex>(need.first),need.second[key]};
  }

  ReadAccess At(const K& key) const{
		auto& need=book[hasher(key)%book.size()];
		return {need.second.at(key),lock_guard<mutex>(need.first),};
  }

  bool Has(const K& key) const{
	 try{
	//  book[hasher(key)%book.size()].second.at(key);
	  At(key);
		 return true;
	 }catch(exception& ex){
		 return false;
...
private:
  Hash hasher;
 mutable vector<pair<mutex,unordered_map<K,V,Hash>>> book;
};
