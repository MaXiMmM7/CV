...
struct AddressHasher {
	...
	size_t operator()(const Address& object) const{
		size_t A = string_hasher(object.city);
		size_t B = string_hasher(object.street);
		size_t C = int_hasher(object.building);
		size_t X= 997;
						return (X*X*A+B*X+C);
	}
...
struct PersonHasher {
	size_t operator()(const Person& object) const{
			size_t A = string_hasher(object.name);
			size_t B = double_hasher(object.weight);
			size_t C = int_hasher(object.height);
			size_t D = address_hasher(object.address);
							size_t X= 997;
