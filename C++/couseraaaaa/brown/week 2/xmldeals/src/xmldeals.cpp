...
vector<Spending> LoadFromXml(istream& input) {
	   vector<Spending> result;
	    ...
	    result.push_back({x.AttributeValue<string>("category"),x.AttributeValue<int>("amount")});
//	    {x.AttributeValue<string>("category")
	//    	    ,AttributeValue<int>("amount")}
	    }
	  return result;
...
void TestXmlLibrary() {
   ...
   * result.push_back(Spending(x.AttributeValue<string>("category")
   * ,AttributeValue<int>("amount")));
   * }
  return result;
  // Реализуйте эту функцию с помощью библиотеки xml.h
