...
struct AddressHasher {
  size_t operator()(const Address& obj) const{
	size_t c=str_hash(obj.city);
	size_t s=str_hash(obj.street);
	size_t b=int_hash(obj.building);
    size_t x = 2'946'901;
	return c*x*x+s*x+b;
  }
...
struct PersonHasher {
	...
	size_t operator()(const Person& p) const{
		size_t n=str_hash(p.name);
		size_t h=int_hash(p.height);
		size_t w=d_hash(p.weight);
		size_t a=address_hasher(p.address);
		size_t x = 2'946'901;

		return x*x*x*n+x*x*h+x*w+a;
	}
