...
/*	Tree(int N):N_(N){
	...
	Budget():N_(65536){
			tree.assign(65536*2,Money());
	}

	/*void Put(int index, double value){
		tree[N_+index]+=value;
		PutParents((N_+index)/2,value);
		//Put(index)
	...
	void Earn(Date from,Date to,int money){
		...
		for(;start<=finish;start++){
			tree[N_+start].earned+=money_per_day;
			PutParentsEarned((N_+start)/2,money_per_day);
		}
	...
	void Spend(Date from,Date to,int money){
		...
		for(;start<=finish;start++){
			tree[N_+start].spend+=money_per_day;
			PutParentsSpend((N_+start)/2,money_per_day);
		}
	...
	void PayTax(Date from, Date to, int percentage = 13){
		...
		double tax = 0;
		for(;start<=finish;start++){
			tax = -((percentage/100.)*tree[N_+start].earned);
			tree[N_+start].earned+=tax;
			PutParentsEarned((N_+start)/2,tax);
		}
	}
	...
	double ComputeIncome(Date from,Date to) const{
		...
		check+=(tree[i].earned-tree[i].spend);
		}
		while(start<=finish){
			if(start % 2 != 0 ){
				sum+=tree[start].earned;
				sum-=tree[start].spend;
			}
			if(finish % 2 == 0){
				sum+=tree[finish].earned;
				sum-=tree[finish].spend;
			}
			start=(start+1)/2;
			finish=(finish-1)/2;
		}
...
private:
	void PutParentsEarned(int index,const double& value){
		if(index>0){
			tree[index].earned+=value;
			PutParentsEarned(index/2,value);
		}
	}

	void PutParentsSpend(int index,const double& value){
		if(index>0){
			tree[index].spend+=value;
			PutParentsSpend(index/2,value);
		}
	}
