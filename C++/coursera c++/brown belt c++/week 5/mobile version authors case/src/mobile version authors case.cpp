...
*/
 ...
 struct BulkTaxApplier {
  ...
  BulkTaxApplier(){}
  BulkTaxApplier(int percentage){
	GetTax(percentage);
  }
  void GetTax(int percentage){
    to_pay*=(1.0 - percentage / 100.0);
  }
...
public:
  ...
  BulkLinearUpdater(const BulkMoneySpend& spend)
      : spend_(spend)
  {}
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    tax_.to_pay *= other.tax_.to_pay;
    spend_.spended+=other.spend_.spended;
    //tax_.to_pay*=other.to_pay;
    add_.delta = add_.delta * other.tax_.ComputeFactor() + other.add_.delta;
  }

  double CollapseIncome(double origin, IndexSegment segment) const {
	  //
//	  cout << "origin: " << origin << " tax: " <<tax_.ComputeFactor() << "  add_delta:  " <<add_.delta << endl;
			  //
    return origin * tax_.ComputeFactor() + add_.delta * segment.length();
  }
  double CollapseSpend(double origin, IndexSegment segment) const {
  	  //
  //	  cout << "origin: " << origin << " tax: " <<tax_.ComputeFactor() << "  add_delta:  " <<add_.delta << endl;
  			  //
    return origin + spend_.spended * segment.length();
   }
private:
  // apply tax first, then add
  BulkTaxApplier tax_;
  BulkMoneyAdder add_;
  BulkMoneySpend spend_;
};
...
private:
  ...
  public:
    ...
    Data ProcessFull(const NodeHolder& node) const {
      return (node->data_income - node->data_spend);
    }
  ...
  public:
    ...
    void ProcessFull(const NodeHolder& node) const {
      ...
      node->data_income = operation_.CollapseIncome(node->data_income, node->segment);
      node->data_spend = operation_.CollapseSpend(node->data_spend, node->segment);
    }

    void ProcessPartial(const NodeHolder& node, IndexSegment) const {
      node->data_income = (node->left ? node->left->data_income : Data()) + (node->right ? node->right->data_income : Data());
      node->data_spend = (node->left ? node->left->data_spend : Data()) + (node->right ? node->right->data_spend : Data());
    }
  ...
  static void PropagateBulkOperation(const NodeHolder& node) {
    for (auto* child_ptr : {node->left.get(), node->right.get()}) {
      if (child_ptr) {
        ...
        child_ptr->data_income = node->postponed_bulk_operation.CollapseIncome(child_ptr->data_income, child_ptr->segment);
        child_ptr->data_spend = node->postponed_bulk_operation.CollapseSpend(child_ptr->data_spend, child_ptr->segment);
      }
    }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier(static_cast<int>(percentage)));
  }
...
struct SpendRequest : ModifyRequest {                     //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneySpend{daily_spend});
  }
