...
//600 0 300 -1000
...

	{
		...
		//dist_dates(gen)
		    ...
		    int Q=100;
		  //  double e=0.0000000000000000000000001;
	...
	//	    double spend_ = 0;
		    ...
		    for(int i = 0; i < Q;i++){
		    	...
		    	budget.Earn(Date(day_from,month_from,year_from),Date(day_to,month_to,year_to),money);
		    	cout << "money : " << money << "  income:  " << sum_ << "  i =  " << i << endl;
		    	//cout << budget.ComputeIncome(Date(1,1,2000),Date(31,12,2099)) << endl;
		...
		sum_ = 0.;
		    ...
		    for(; i < Q;i++){
		  		    	...
		  		    	budget.Spend(Date(day_from,month_from,year_from),Date(day_to,month_to,year_to),money);
		  		    	cout << "money : " << money << "  income:  " << sum_ << "  i =  " << i << endl;
		  		    	//cout << budget.ComputeIncome(Date(1,1,2000),Date(31,12,2099)) << endl;
		  		    	stream1 <<fixed <<   setprecision(25) << budget.ComputeIncome(Date(1,1,2000),Date(31,12,2099));
		  		    	stream2 <<fixed <<    setprecision(25) << sum_;
		  		    	stream1 >> res1;
		  		    	stream2 >> res2;
		  		    	cout << "res1:  " << res1 << "  res:2  " << res2<< endl;
		  		    	ASSERT_EQUAL(((res1-res2)<std::numeric_limits<double>::epsilon()),true);
		  		    }
