...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor,tree_spend,tree_values_spend;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  ...
  if (ql <= l && r <= qr) {
	  //
	  //cout << tree_values[v] << " " <<  tree_spend[v] << endl;
	//
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
	//1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from)
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr,double value) {
  ...
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    tree_factor[v] *= (1.-(value/100.));
    tree_add[v] *= (1.-(value/100.));
    tree_values[v] *= (1.-(value/100.));
    return;
  }
