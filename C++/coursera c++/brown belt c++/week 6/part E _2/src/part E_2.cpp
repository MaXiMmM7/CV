...
void ProcessJson(istream& input = cin,ostream& output = cout){
	...
	auto root = Doc.GetRoot().AsMap();
		...
		vector<string> stop_by_id;
		unordered_map<string,size_t> id_by_stop;
		...
		for(auto [bus,bus_ptr]: buses_park){
			...
			for(int i = 0; i < static_cast<int>(stops_for_bus.size()); i++){
				if(id_by_stop.count(stops_for_bus[i]->GetName()) == 0){
					stop_by_id.push_back(stops_for_bus[i]->GetName());
					id_by_stop[stops_for_bus[i]->GetName()] = helper_id++;
				}
				//
							cerr << "new name "  << stops_for_bus[i]->GetName()
									<< " with id: " <<  id_by_stop[stops_for_bus[i]->GetName()] << endl;
							//

				if(i > 0){
					dist = stops.GetStop(stops_for_bus[i-1]->GetName()).value()->GetDistTo(stops_for_bus[i]->GetName()).value();
					time = dist/bus_velocity;
					graph.AddEdge({(id_by_stop[stops_for_bus[i-1]->GetName()]), id_by_stop[stops_for_bus[i]->GetName()], time});
				}
			}
		}
		...
		for(int i = 0 ;i < static_cast<int>(stat_requests.size()); i++ ){
			...
			if(request["type"].AsString()=="Bus"){
				...
				if(built_route.has_value()){
					...
					set<string> prev,now,intersect_was,intersect_now,intersect;
				//	string str;
					double local_time = 0.;
					int span_count = 0;

					auto first_edge = graph.GetEdge(router.GetRouteEdge(built_route.value().id, 0));
					prev=stops.GetStop(stop_by_id[first_edge.from]).value()->GetBuses();
					now=stops.GetStop(stop_by_id[first_edge.to]).value()->GetBuses();
					set_intersection(prev.begin(),prev.end(),now.begin(),now.end(),
							inserter(intersect_was,intersect_was.end()));
					output << "\n{\n\"type\": \"Wait\",\n\"stop_name\": \"" << from_
							<< "\",\n\"time\": " << wait_time  << "\n},";
					...
					for(int i = 0; i < static_cast<int>(built_route.value().edge_count); i++){
						...
						prev=stops.GetStop(stop_by_id[edge.from]).value()->GetBuses();
						now=stops.GetStop(stop_by_id[edge.to]).value()->GetBuses();
						set_intersection(prev.begin(),prev.end(),now.begin(),now.end(),
								inserter(intersect_now,intersect_now.end()));
						set_intersection(intersect_was.begin(),intersect_was.end(),
								intersect_now.begin(),intersect_now.end(),
