...
struct Double_2{
	Double_2(double t = 0., int s_c = 0):time_(t),stop_count(s_c){}
	double time_;
	int stop_count;
};
...
bool operator==(const Data& l, const Data& r){
	return (l.stop_name==r.stop_name)&&(l.type==r.type)&&(l.bus==r.bus);
}
	...
	//	cerr << bus_velocity << " " << wait_time << endl;
		...
		for(auto [bus,bus_ptr]: buses_park){
			...
			for(int i = 0; i < static_cast<int>(stops_for_bus.size()); i++){
				if(id_by_stop.count({bus,stops_for_bus[i]->GetName(),Type::wait}) == 0){
					stop_by_id.push_back({bus,stops_for_bus[i]->GetName(),Type::wait});
					id_by_stop[{bus,stops_for_bus[i]->GetName(),Type::wait}]=(stop_by_id.size()-1);
					stop_by_id.push_back({bus,stops_for_bus[i]->GetName(),Type::bus});
					id_by_stop[{bus,stops_for_bus[i]->GetName(),Type::bus}]=(stop_by_id.size()-1);
				}
			}
		}
...
//
		...
		//		cerr << "\n STOP IS " << stops_for_bus[i]->GetName() << endl << endl;
				//
				graph.AddEdge({id_by_stop[{bus,stops_for_bus[i]->GetName(),Type::wait}],
					id_by_stop[{bus,stops_for_bus[i]->GetName(),Type::bus}], {wait_time, 0}});
				//
			...
			//		cerr << " dist is " << dist << " bus_velocity is " << bus_velocity  << " time is " << time << endl;
				...
				//							cerr << "from: " << stops_for_bus[j]->GetName() << " to: "
					//								<< stops_for_bus[i]->GetName() << " time: " <<   sum <<
						...
						//
						graph.AddEdge({id_by_stop[{bus,stops_for_bus[j]->GetName(),Type::bus}],
												id_by_stop[{bus,stops_for_bus[i]->GetName(),Type::wait}], {sum,(i-j)}});
					}
				...
				for(auto [stop_name,stop_ptr]: stops_helper){
					...
					for(int i = 0; i+1 < static_cast<int>(stop_buses_book.size()); i++){
						for(int j = i+1; j < static_cast<int>(stop_buses_book.size()); j++){
							//
							//cerr <<"we are inside " << endl;
							//
									graph.AddEdge({(id_by_stop[{stop_buses_book[i],stop_name,Type::wait}]),
								id_by_stop[{stop_buses_book[j],stop_name,Type::wait}], {0.,0}});
							graph.AddEdge({(id_by_stop[{stop_buses_book[j],stop_name,Type::wait}]),
								id_by_stop[{stop_buses_book[i],stop_name,Type::wait}], {0.,0}});
							}
							//
		...
		for(int i = 0 ;i < static_cast<int>(stat_requests.size()); i++ ){
			...
			//	cerr << "hey 1 "   << from_ << "  " << to_<< endl;
				...
				if((!book_from.empty())&&(!book_to.empty())){
					auto built_route = router.BuildRoute(id_by_stop[{*book_from.begin(),from_,Type::wait}],
							id_by_stop[{*book_to.begin(),to_,Type::wait}]);

					for(auto from_bus: book_from){
						for(auto to_bus: book_to){
							auto helper = router.BuildRoute(id_by_stop[{from_bus,from_,Type::wait}],
									id_by_stop[{to_bus,to_,Type::wait}]);
							if(helper.has_value()){
